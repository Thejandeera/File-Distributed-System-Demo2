...
func main() {
	...
	servers := make([]*goraft.Server, len(cluster))
	sms := make([]*kvStateMachine, len(cluster))
	for i := range cluster {
		sms[i] = newKvSM()
		servers[i] = goraft.NewServer(cluster, sms[i], ".", i)
	}
	...
	restartCluster(servers, cluster, sms, nil)
	...
	restartCluster(servers, cluster, sms, func() {
		os.Remove(servers[len(servers)-1].Metadata())
	})
...
func restartCluster(servers []*goraft.Server, cluster []goraft.ClusterMember, sms []*kvStateMachine, preStart func()) {
	for _, s := range servers {
		s.Shutdown()
	}
